// Validar cookies de login
//if(document.cookie != "user=ajovel"){location="login.html"}
 
// Borrar cookies login
function borrarcookie(){
      var d = new Date();
      document.cookie = "user=1;expires=" + d.toGMTString() + ";" + ";";
 }

// detectar inactividad (60 segundos)
...
$(function	()	{
	...
	$('.upload-demo').change(function()	{
		...
		$(this).parent().find('label').attr('data-title','Cambiar archivo');
		$(this).parent().find('label').addClass('selected');
	});

	$('.remove-file').click(function()	{
		$(this).parent().find('span').attr('data-title','No hay archivo...');
		$(this).parent().find('label').attr('data-title','SAlbumSete archivo');
